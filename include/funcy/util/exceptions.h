...
namespace funcy
{
    ...
    public:
        ...
        OutOfDomainException( const std::string& function, const std::string& range,
                              const Value& value, const std::string& file, const int line )
            : std::runtime_error( std::string( "OutOfDomainException in " ) + file + " at line " +
                                  std::to_string( line ) + ".\n           " + function +
                                  ": Argument " + std::to_string( value ) + " is outside range " +
                                  range + ".\n" )
        {
        }
    };
    ...
    public:
         ...
         */
        template < class Value, class = std::enable_if_t< std::is_arithmetic< Value >::value > >
        NonSymmetricMatrixException( const std::string& function, const Value& rows,
                                     const Value& cols, const std::string& file, const int line )
            : std::runtime_error( std::string( "NonSymmetricMatrixException in " ) + file +
