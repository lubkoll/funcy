...
namespace funcy
{
    namespace static_check
    {
        ...
        struct HasMemFn_nrows : std::false_type
        {
        };

        template < class Matrix >
        struct HasMemFn_nrows< Matrix, std::void_t< TryMemFn_rows< Matrix > > > : std::true_type
        {
        };

        template < class Matrix, class = void >
        struct HasMemFn_cols : std::false_type
        {
        };
        template < class Matrix >
        struct HasMemFn_cols< Matrix, std::void_t< TryMemFn_cols< Matrix > > > : std::true_type
        {
        };

        template < class Matrix, class = void >
        struct HasMem_n_rows : std::false_type
        {
        };
        template < class Matrix >
        struct HasMem_n_rows< Matrix, std::void_t< TryMem_n_rows< Matrix > > > : std::true_type
        {
        };

        template < class Matrix, class = void >
        struct HasMem_n_cols : std::false_type
        {
        };

        template < class Matrix >
        struct HasMem_n_cols< Matrix, std::void_t< TryMem_n_cols< Matrix > > > : std::true_type
        {
        };

        template < class Vector, class = void >
        struct HasMemFn_size : std::false_type
        {
        };

        template < class Vector >
        struct HasMemFn_size< Vector, std::void_t< TryMemFn_size< Vector > > > : std::true_type
        {
        };
        /// @endcond
