...
{
  /// @cond
  namespace Concepts { template <class> struct IsMatrix; }
  /// @endcond

  namespace linalg
  {
    /// Specialize this for your matrix class. Number of rows must be provided by a static member variable called value.
    template < class Matrix , class = Concepts::IsMatrix<Matrix> > struct NumberOfRows : std::integral_constant<int,-1> {};

    /// Specialize this for your matrix class. Number of columns must be provided by a static member variable called value.
    template < class Matrix , class = Concepts::IsMatrix<Matrix> > struct NumberOfColumns : std::integral_constant<int,-1> {};
