  ...
  namespace linalg
  {
    ...
    namespace Detail
    {
      template <class Matrix, class TransposedMatrix = typename GetTransposed<Matrix>::type,
                std::enable_if_t<std::is_same<Matrix,TransposedMatrix>::value && Concepts::isConstantSize<Matrix>()>* = nullptr>
    ...
    private:
      ...
      typename std::conditional< Concepts::isConstantSize<Matrix>(), GetTransposed<Matrix>, WrappedMatrix >::type::type AT_;
    };
