...
namespace funcy
{
    ...
    auto incompressibleMooneyRivlin( double c0, double c1, const M& F )
    {
    ...
    template < class InflationPenalty, class CompressionPenalty, Matrix M,
               int n = linalg::dim< M >() >
    auto compressibleMooneyRivlin( double c0, double c1, double d0, double d1, const M& F )
    {
        using namespace linalg;
        auto S = LeftCauchyGreenStrainTensor( F );
        auto f = ( c0 * ( i1( F ) - n ) + c1 * ( i2( F ) - n ) );
    ...
    //   */
    //  template <Matrix M, class InflationPenalty, class CompressionPenalty>
    ...
    //      std::endl; std::cout << "Computed parameters: " << alpha << ", " << beta<< ", " << c <<
    //      ", " << d << std::endl; std::cout << "Material law is not polyconvex! -> Terminating."
    //      << std::endl; exit(1);
    //    }
