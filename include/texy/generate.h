...
template <class F0, class G0,
          bool = funcy::Concepts::isFunction<std::decay_t<F0>>(),
          bool = funcy::Concepts::isFunction<std::decay_t<G0>>()>
struct SumGenerator;
...
template <
    class F, class G,
    std::enable_if_t<funcy::Concepts::isFunction<std::decay_t<F>>() ||
                     funcy::Concepts::isFunction<std::decay_t<G>>()> * = nullptr>
 ...
 * wrap it into Finalize to generate missing derivatives.
 */
template <class F, std::enable_if_t<funcy::Concepts::isFunction<std::decay_t<F>>()>
                       * = nullptr>
...
//               std::enable_if_t< funcy::Concepts::isFunction< std::decay_t< F >
//               >() &&
//                                 funcy::Concepts::isFunction< std::decay_t< G >
//                                 >() >* = nullptr
//                                 >
//    auto operator<<( F&& f, G&& g )
//    {
