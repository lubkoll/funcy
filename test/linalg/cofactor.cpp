...
namespace
{
    ...
    template < int i, int j >
    auto symmetric_derivative_x2d( const MXd& A, const MXd& B )
    {
        return compute_cofactor_directional_derivative< i, j >( A, B );
    }
    ...
    template < int i, int j >
    auto symmetric_derivative_x3d( const MXd& A, const MXd& B )
    {
        return compute_cofactor_directional_derivative< i, j >( A, B ) +
               compute_cofactor_directional_derivative< i, j >( B, A );
    }
...
TEST( DynamicCofactorTest_2d, D0 )
{
    const auto A = get_first_matrix_x2d();
...
TEST( DynamicCofactorTest_2d, D1 )
{
    const auto A = get_first_matrix_x2d();
    const auto dA = get_second_matrix_x2d();
    auto value = symmetric_derivative_x2d< 0, 0 >( A, dA );
    EXPECT_THAT( value, DoubleEq( dA( 1, 1 ) ) );

    value = symmetric_derivative_x2d< 0, 1 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -dA( 1, 0 ) ) );

    value = symmetric_derivative_x2d< 1, 0 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -dA( 0, 1 ) ) );

    value = symmetric_derivative_x2d< 1, 1 >( A, dA );
    EXPECT_THAT( value, DoubleEq( dA( 0, 0 ) ) );
}

TEST( StaticCofactorTest_2d, D0 )
{
...
TEST( DynamicCofactorTest_3d, D0 )
{
    const auto A = get_first_matrix_x3d();
...
TEST( DynamicCofactorTest_3d, D1 )
{
    const auto A = get_first_matrix_x3d();
    const auto dA = get_second_matrix_x3d();
    auto value = symmetric_derivative_x3d< 0, 0 >( A, dA );
    EXPECT_THAT( value, DoubleEq( A( 1, 1 ) + A( 2, 2 ) ) );

    value = symmetric_derivative_x3d< 0, 1 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 1, 0 ) ) );

    value = symmetric_derivative_x3d< 0, 2 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 2, 0 ) ) );

    value = symmetric_derivative_x3d< 1, 0 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 0, 1 ) ) );

    value = symmetric_derivative_x3d< 1, 1 >( A, dA );
    EXPECT_THAT( value, DoubleEq( A( 0, 0 ) + A( 2, 2 ) ) );

    value = symmetric_derivative_x3d< 1, 2 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 2, 1 ) ) );

    value = symmetric_derivative_x3d< 2, 0 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 0, 2 ) ) );

    value = symmetric_derivative_x3d< 2, 1 >( A, dA );
    EXPECT_THAT( value, DoubleEq( -A( 1, 2 ) ) );

    value = symmetric_derivative_x3d< 2, 2 >( A, dA );
    EXPECT_THAT( value, DoubleEq( A( 0, 0 ) + A( 1, 1 ) ) );
}
